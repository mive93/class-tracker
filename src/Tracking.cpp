...
void Tracking::kalmanStep(const std::vector<obj_m>& new_trajs){
    ...
    for (size_t i = 0; i < new_trajs.size(); i++){
        ...
        trackers[i].trajEkf.push_back(obj_m(trackers[i].ekf.getEstimatedState().x,trackers[i].ekf.getEstimatedState().y, trackers[i].traj.back().frame, trackers[i].traj.back().cl, 0, 0));
    }
}
